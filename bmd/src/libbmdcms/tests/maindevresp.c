...
/*	{
		DVCSResponse_t *dvcsresp = NULL;
		wynik(DVCSResponse_generate_ErrorNotice(PKIFailureInfo_badRequest,NULL, &dvcsresp));
		wynik(CMSDVCSResponse_sign(dvcsresp, &cert, &privKey, &outdvcs));
	}*/

	{
		DVCSRequest_t *dvcsreq = NULL;
		DVCSResponse_t *dvcsresp = NULL;
		DVCSTime_t *dvcstime = NULL;
		struct tm *tm_s;
		time_t curtime;
		wynik(DVCSRequest_VSD(
			&cert, &privKey, outbuf, 0, &outdvcs));
		wynik(CMSDVCSRequest_Get_DVCSRequest(
			outdvcs, &dvcsreq));
		dvcstime = mallocStructure(sizeof(DVCSTime_t));
		dvcstime->present = DVCSTime_PR_genTime;
		time(&curtime);
		tm_s = gmtime( &curtime );
		asn_time2GT(&(dvcstime->choice.genTime), tm_s, 0);
		wynik(DVCSResponse_generate_DVC(
			dvcsreq, 10, dvcstime, DVCSHASH_SHA1, &dvcsresp));
		wynik(free_GenBuf(&outdvcs));
		outdvcs = NULL;
		wynik(CMSDVCSResponse_sign(dvcsresp, &cert, &privKey, &outdvcs));
	}

	{
		...
		wynik(DVCSResponse_generate_DVC(
			dvcsreq, 10, dvcstime, DVCSHASH_SHA1, &dvcsresp));
		wynik(DVCSResponse_set_PKIStatus(dvcsresp, PKIStatus_accepted, 0, 0));
		wynik(free_GenBuf(&outdvcscert));
		outdvcscert = NULL;
		wynik(CMSDVCSResponse_sign(dvcsresp, &cert, &privKey, &outdvcscert));
	}
	...
	wynik(cryptEnd());
	{
		FILE *plik = fopen("dvcsresp.sig", "wb");
		fwrite(outdvcs->buf, 1, outdvcs->size, plik);
		fclose(plik);
		plik = fopen("dvcsrespcert.sig", "wb");
