...
int main(int argc, char ** argv)
{
	...
	assert(bmd_datagram_create(BMD_DATAGRAM_TYPE_CERTIFY_DVCS, &dtg) == BMD_OK);


	assert(set_gen_buf2(dvcs_id, strlen(dvcs_id), &DVCS_Id) == BMD_OK);
	dtg->protocolData=DVCS_Id;
