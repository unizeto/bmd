...
int main(int argc, char ** argv)
{
	...
	assert(bmd_datagram_create(BMD_DATAGRAM_TYPE_CERTIFY_TIMESTAMP, &dtg) == BMD_OK);


	assert(set_gen_buf2(timestamp_id, strlen(timestamp_id), &TIMESTAMP_Id) == BMD_OK);
	dtg->protocolData=TIMESTAMP_Id;
