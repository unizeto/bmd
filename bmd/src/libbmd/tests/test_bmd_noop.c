...
int main(int argc, char ** argv)
{
	...
	for (i=0; i<4; i++)
	{
		FOK(bmd_datagram_create(BMD_DATAGRAM_TYPE_NOOP,&dtg));
	...
	{
		printf("%i, %li \n",(unsigned char *)resp_dtg_set->bmdDatagramSetTable[i]->datagramType,resp_dtg_set->bmdDatagramSetTable[i]->datagramStatus);
