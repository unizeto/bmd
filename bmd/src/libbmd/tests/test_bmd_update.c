	...
	//	{
			...
			for (u=0; u<1; u++)
			{
				...
				//FOK(bmd_lob_request_info_add_filename(L"/home/Lesioo/Lesioo/trunk/src/libbmd/tests/cos.doc",&li, BMD_SEND_LOB,0));
					...
					FOK(bmd_datagram_create(BMD_DATAGRAM_TYPE_UPDATE,&dtg));
					...
					for (i=0; i<1; i++)
					{
						switch (i)
						{
							case 0:
								...
								tmp_gb1.buf="paw.lesniak@gmail.com";
					...
					asprintf(&sort_str, "Powód modyfikacji: bo tak... a co nie mogę?");
			...
			//printf("%s:%i:%s PKL\n",__FILE__,__LINE__,__FUNCTION__);
	status=bmd_send_request(bi,req_dtg_set, 100, &resp_dtg_set,0);

	printf("\n\n\n\nresponse\n------------------- %i\n",status);
	BMD_FOK(resp_dtg_set->bmdDatagramSetTable[0]->datagramStatus);

	for (i=0; i<resp_dtg_set->bmdDatagramSetSize; i++)
	{
		printf("%i. %i \n",i,resp_dtg_set->bmdDatagramSetTable[i]->datagramStatus);
	}
