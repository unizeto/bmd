...
long __bmdnet_socks4_create(	bmdnet_handler_ptr handler)
{
...
/** @test untested */
long __bmdnet_socks4_connect(	bmdnet_handler_ptr handler,
					char*host,
					long port)
{
	char * buffer				= NULL;
	...
	request_header.destaddr=inet_addr(host/*(char *)handler->data[PROXY_SOCKS_DEST_ADDR]*/);
	...
	if(handler->data[PROXY_USERNAME])
	{
		...
		bmdnet_send(handler->data[CONNECTION], complete_request,length+request_header_size+1);
	}
	else
	{
		bmdnet_send(handler->data[CONNECTION],complete_request,request_header_size+1);
	}
...
long __bmdnet_socks4_send(	bmdnet_handler_ptr handler,
					void * buffer,
					long buffer_size)
{
...
long __bmdnet_socks4_recv(bmdnet_handler_ptr handler, void * buffer,long buffer_size, 
	long blocking)
{
...
long __bmdnet_socks4_close(	bmdnet_handler_ptr handler)
{
	long status = 0;
...
/** Ustawianie opcji proxy typu SOCKS4
...
long __bmdnet_socks4_set_option(	bmdnet_handler_ptr handler, 
						const long option,
...
#ifndef WIN32
const bmdnet_method_t __bmdnet_method_SOCKS4 = {
	BMD_NET_SOCKS4_METHOD,

	NULL, /*  _bmdnet_plaintcp_init, */
	NULL, /*__bmdnet_plaintcp_destroy, */

	__bmdnet_socks4_create,
	__bmdnet_socks4_connect,
	__bmdnet_socks4_close,

	__bmdnet_socks4_send,
	__bmdnet_socks4_recv,

	NULL, /* _bmdnet_plaintcp_bind, */
	NULL, /* __bmdnet_plaintcp_accept */
	__bmdnet_socks4_set_option,
};
#else

bmdnet_method_t __bmdnet_method_SOCKS4;
void init_bmdnet_method_SOCKS4()
{
	bmdnet_method_t *tmp_method=NULL;
	tmp_method=(bmdnet_method_t *)malloc(sizeof(bmdnet_method_t));

	tmp_method->id=BMD_NET_SOCKS4_METHOD;
	tmp_method->init=NULL;
	tmp_method->destroy=NULL;
	tmp_method->create=__bmdnet_socks4_create;
	tmp_method->connect=__bmdnet_socks4_connect;
	tmp_method->close=__bmdnet_socks4_close;
	tmp_method->send=__bmdnet_socks4_send;
	tmp_method->recv=__bmdnet_socks4_recv;
	tmp_method->bind=NULL;
	tmp_method->accept=NULL;
	tmp_method->set_option = __bmdnet_socks4_set_option;

	memmove(&__bmdnet_method_SOCKS4,tmp_method,sizeof(bmdnet_method_t));
}
