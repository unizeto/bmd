...
server_request_data_t *global_server_request_data=NULL;
...
char *server_info_postgres =	"hostaddr='127.0.0.1' dbname='bmd' port='5432' user='bmd' password='12345678'";
...
/*	bmd_db_connect(server_info_postgres, &db_conn_info);*/
	...
	/*dg1->datagramType = BMD_DATAGRAM_TYPE_GET_PKI;*/
...
/*	strcpy(dg.protocolDataFileId->buf, "1282");*/
	strcpy(dg1->protocolDataFileId->buf, file_id);
...
/*	PR_bmdDatagramSet_print(set1, WITHOUT_TIME);*/
	...
	FOK(bmd_server_configuration_create(&_GLOBAL_bmd_configuration));
	FOK(bmd_get_server_configuration(bmd_conf_file,_GLOBAL_bmd_configuration, TIME_FIRST));

/*	server_request_data_t req;
	req.js_ret.server_ctx=_GLOBAL_bmd_configuration->crypto_conf->server_ctx;
	err=bmd_set_ctx_cert(&(req.userCertificate),req.X509_buf);
	CHECK_REQUEST_SESSION_HANDLE_STATUS(err);
	req.js_ret.user_cert_ctx=req.userCertificate;
	err=JS_main(req.db_conn_info,req.request_datagram,&(req.js_datagram),handler, &(req.js_ret),&data);
	CHECK_REQUEST_SESSION_HANDLE_STATUS(err);
	status=BMD_encode_and_send_response(handler,&req,req.userCertificate,_GLOBAL_bmd_configuration->crypto_conf->server_ctx);
	CHECK_REQUEST_SESSION_HANDLE_STATUS(status);
*/

	err = JS_main(db_conn_info, set1, &set2, NULL, &js_ret, NULL);
	if(err<0){
...
/*	PR_bmdDatagramSet_print(set2, WITHOUT_TIME);*/
	...
	ConfigData_free(&(_GLOBAL_bmd_configuration->ConfigData));
	return err;
}
