...
int j=0;
	...
	/*********************************************************************************/
	/************************ funkcja testowa ****************************************/
	status=soap_call_bmd230__bmdGetRolesList(		&soap, host, NULL, base64Cert, NULL, NULL,NULL, strcmp(roleId, "0")==0?NULL:roleId, &result);
	/*********************************************************************************/
	/*********************************************************************************/
	if (status == SOAP_OK)
	{
		...
		{
			printf("-------------------------------------------------------------------\n");
			printf("roleId:\t\t\t%s\n",result->__ptr[i].roleId);
			printf("roleName:\t\t%s\n",result->__ptr[i].roleName);
			if (result->__ptr[i].roleRightsOids!=NULL)
			{
				printf("role rigths list:\n");
				for (j=0; j<result->__ptr[i].roleRightsOids->__size; j++)
				{
					printf("\t%s    %s\n", result->__ptr[i].roleRightsOids->__ptr[j], result->__ptr[i].roleRightsCodes->__ptr[j]);
				}
			}
			if (result->__ptr[i].roleActionList!=NULL)
			{
				printf("action list:\n");
				for (j=0; j<result->__ptr[i].roleActionList->__size; j++)
				{
					printf("\t%s\n", result->__ptr[i].roleActionList->__ptr[j]);
				}
			}
			printf("\n");
		}
