...
FILE* fd2 = NULL;
	...
	else
	{
		...
		printf("\t-p id\t\tid pliku, do którego ma być dodany podpis\n");
		printf("\t-s file\t\tpodpis do dodania do bazy\n");
		printf("-------------------------------------------------------\n");
		return -1;
	}
	...
	base64Signature = (struct xsd__base64Binary *)malloc(sizeof(struct xsd__base64Binary));
	...
	base64Cert->__size=cert_pem->size;
		    ...
		    bmd_load_binary_content(sig,&podpis);
                base64Signature->__ptr = podpis->buf;
	...
	/*********************************************************************************/
	/************************ funkcja testowa ****************************************/
	status = soap_call_bmd230__bmdAddSignature(		&soap, host, NULL, base64Cert, NULL, NULL, NULL, base64Signature, strtol(file, NULL, 10), &result);
// 	status = soap_call_bmd230__bmdInsertSignedFile(&soap, host, NULL, base64Cert, mtds, input, base64Signature, NULL, NULL, transactionId, nostream, &result);
	/*********************************************************************************/
	/*********************************************************************************/
	...
	if (status == SOAP_OK)
	{
		/*********************************************************************************/
		/************************ funkcja odpowiedź ****************************************/
		printf("Podpis zapisany w archiwum; result %i\n",result);
