...
int i=0;
	...
	certificateAttributes->userCertSerial="a1d4b2b2dfdsds";
	certificateAttributes->userName="nowy certyfikat";
	certificateAttributes->userIdentity="tożsamość z opisem";
...
// 		certificateAttributes->userNewGroupInfo->userNewGroupChildIds->__ptr=(xsd__string *)malloc(sizeof(xsd__string)*certificateAttributes->userNewGroupInfo->userNewGroupChildIds->__size);
	...
	/*********************************************************************************/
	/************************ funkcja testowa ****************************************/
	status=soap_call_bmd230__bmdAddCertificateToIdentity(&soap, host, NULL, base64Cert, NULL, NULL, NULL, "2", certificateAttributes, &result);
	...
	if (status == SOAP_OK)
	{
		...
		/*********************************************************************************/
		/************************ funkcja odpowiedź ****************************************/
		printf("Dodanie certyfikatu dla tożsamości powiodło się %i\n",result);
