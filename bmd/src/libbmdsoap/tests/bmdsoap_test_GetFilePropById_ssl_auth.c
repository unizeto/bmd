...
int option=0;
	...
	if (argc==7)
	{
		for (i=1; i<argc; i++)
		{
			...
			{
				if (argc>i+1) option=atoi(argv[i+1]);
			}
		}
	}
	else
	{
		...
		printf("\t-p liczba\t\tid pliku\n");
		printf("\t-r przełącznik\t(-2-Signature XAdES; -1-DVCS; 0-Signature CMS; 1...n-Timestamp)\n");
		printf("-------------------------------------------------------\n");
		return -1;
	}
	...
	/*********************************************************************************/
	/************************ funkcja testowa ****************************************/
	status=soap_call_bmd230__bmdGetFilePropByID(&soap, host, NULL, base64Cert, NULL, NULL, NULL, id, option, &fileResponse);
	//xsd__int bmd230__bmdGetFilePropByID(struct soap *soap,char *ID,int property,struct xsd__base64Binary *base64Cert,struct bmd230__fileInfo **output)
	/*********************************************************************************/
	/*********************************************************************************/
