...
struct bmd230__historyResults *historyResults = NULL;
...
long int id=0;
	...
	/*********************************************************************************/
	/************************ funkcja testowa ****************************************/
	status=soap_call_bmd230__bmdHistory(&soap, host, NULL, base64Cert, NULL, NULL,NULL, id,&historyResults);
	/*********************************************************************************/
	/*********************************************************************************/
	...
	if (status == SOAP_OK)
	{
		/*********************************************************************************/
		/************************ funkcja odpowiedź ****************************************/
		printf("Znalezionych dokumentów: %i\n",historyResults->__size);
 		int i=0;
 		for (i=0; i<historyResults->__size; i++)
 		{
 			printf("\n\n\n%s \n",historyResults->__ptr[i].updateDate);
			printf("%s \n",historyResults->__ptr[i].updateOwner);
			printf("%s \n",historyResults->__ptr[i].updateReason);

 			int j=0;
 			for (j=0; j<historyResults->__ptr[i].mtds->__size; j++)
 			{
 				printf("\t\t%s\t\t%s\t\t%s\n",historyResults->__ptr[i].mtds->__ptr[j].mtdOid,historyResults->__ptr[i].mtds->__ptr[j].mtdDesc,historyResults->__ptr[i].mtds->__ptr[j].mtdValue);
 			}
 		}
