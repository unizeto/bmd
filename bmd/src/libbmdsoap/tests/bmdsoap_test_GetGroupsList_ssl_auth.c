...
int j=0;
	...
	/*********************************************************************************/
	/************************ funkcja testowa ****************************************/
	status=soap_call_bmd230__bmdGetGroupsList(		&soap, host, NULL, base64Cert, NULL, NULL,NULL, groupId, &result);
	/*********************************************************************************/
	/*********************************************************************************/
	if (status == SOAP_OK)
	{
		/*********************************************************************************/
		/************************ funkcja odpowiedź ****************************************/
		printf("Pobrano listę grup %i:\n",result->__size);

		for (i=0; i<result->__size; i++)
		{
			printf("-------------------------------------------------------------------\n");
			printf("groupId:\t\t\t%s\n",result->__ptr[i].groupId);
			printf("groupName:\t\t%s\n",result->__ptr[i].groupName);
			if (result->__ptr[i].groupParentNames)
			{
				printf("group parents list:\n");

				for (j=0; j<result->__ptr[i].groupParentNames->__size; j++)
				{
					printf("\t%s\t%s\n", result->__ptr[i].groupParentIds->__ptr[j], result->__ptr[i].groupParentNames->__ptr[j]);
				}
			}
			if (result->__ptr[i].groupChildNames)
			{
				printf("children list:\n");
				for (j=0; j<result->__ptr[i].groupChildNames->__size; j++)
				{
					printf("\t%s\t%s\n", result->__ptr[i].groupChildIds->__ptr[j], result->__ptr[i].groupChildNames->__ptr[j]);
				}
			}
			printf("\n");
		}
