...
int stream = 0;
	...
	if (argc==5)
	{
		for (i=1; i<argc; i++)
		{
			...
			{
				if (argc>i+1) asprintf(&getReason,"%s",argv[i+1]);
				printf("%s\n",userIndentifier);
			}
		}
	}
...
printf("invoiceId: %s\n", invoiceId);
                /*********************************************************************************/
                /************************ funkcja testowa ****************************************/
                //	status=soap_call_bmd230__getInvoicePack(&soap, host, NULL, accountId, invoiceId, base64Cert, NULL, NULL, &fileResponse);
		status=soap_call_bmd230__getInvoicePackById( &soap, host, NULL, base64Cert,  NULL, NULL, NULL, strtol(invoiceId, NULL, 10), &fileResponse);
//                 status=soap_call_bmd230__getInvoicePack(&soap, host, NULL, accountId, invoiceId, userIndentifier, getReason, base64Cert, NULL, NULL, nostream, &fileResponse);
                /*********************************************************************************/
                /*********************************************************************************/
...
//
//                  status=soap_call_bmd230__getInvoicePack(&soap, host, NULL,base64Cert, NULL, NULL, accountId, invoiceId, userIndentifier, getReason, stream, &fileResponse);
//
//                  if (status == SOAP_OK)
//                   {
//                    /*********************************************************************************/
