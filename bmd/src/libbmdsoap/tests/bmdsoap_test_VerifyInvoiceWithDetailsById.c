...
GenBuf_t plik;
int stream = 0;
struct bmd230__stringList *result=NULL;
struct bmd230__fileInfoList *output=NULL;
int forceverification=0;
int completeverification=0;
	...
	if (argc==11)
	{
		for (i=1; i<argc; i++)
		{
			...
			{
				if (argc>i+1) completeverification=atoi(argv[i+1]);
			}
		}
	}
	else
	{
		...
		printf("\t-f liczba\tforceverification\n");
		printf("\t-c liczba\tcompleteverification\n");
	...
	switch(stream)
	{
		case 0 :
			{
				/*********************************************************************************/
				/************************ funkcja testowa ****************************************/
				status=soap_call_bmd230__verifyInvoiceWithDetailsById(	&soap, host, NULL, base64Cert, NULL, NULL, NULL, id, forceverification, completeverification, &output);
				/*********************************************************************************/
				/*********************************************************************************/
				...
				/*********************************************************************************/
				/************************ funkcja odpowiedź ****************************************/
					printf("Odpowiedź serwera: %i\n-------------------------\n", output->__size);
					for (i=0; i<output->__size; i++)
					{
						printf("%i. dvcs binary size: %i\n", i, output->__ptr[i].file->__size);
					}
					printf("\n\n");
     ...
     case 1 : {
                ...
                /*********************************************************************************/
                /************************ funkcja testowa ****************************************/
                 status=soap_call_bmd230__bmdGetFileById(&soap, host, NULL, base64Cert, NULL, NULL, NULL, id, stream, &fileResponse);
                /*********************************************************************************/
                /*********************************************************************************/
