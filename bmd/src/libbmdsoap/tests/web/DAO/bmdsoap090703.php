...
//draw_htmlhead('Komunikat SOE SOAP WWW','');//do testow 
...
class BMDSOAP {
    ...
    function __construct() {
        ...
        $this->client = new SoapClient("./lib/ns090703.wsdl",array("trace" => 1 ,
     ...
     * @returns tablicę obiektów zawierających identyfikatory OID i opisy metadanych dokumentów
     */
    function bmdSearchFiles($cert, $mtds, $formMtds, $offset,$limit, $logical, $oper, $range, $sortInfo) {
        try {
            $params = array("cert"=>$cert,"mtds"=>$mtds,"formMtds"=>$formMtds,  "offset"=>$offset, "limit"=>$limit, "logical"=>$logical, "oper"=>$oper, "range"=>$range, "sortInfo"=>$sortInfo);
...
/*
 * 
 * <element name="cert" type="xsd:base64Binary" minOccurs="0" maxOccurs="1" nillable="true"/>
     <element name="mtds" type="ns:mtdsSearchValues" minOccurs="0" maxOccurs="1" nillable="true"/>
     <element name="formMtds" type="ns:mtdsValues" minOccurs="0" maxOccurs="1" nillable="true"/>
     <element name="offset" type="xsd:int" minOccurs="1" maxOccurs="1"/>
     <element name="limit" type="xsd:int" minOccurs="1" maxOccurs="1"/>
     <element name="sortInfo"
 */
    function bmdAdvancedSearchFiles($cert, $mtds, $formMtds, $offset,$limit, $sortInfo) {
        try {
            $params = array("cert"=>$cert,"mtds"=>$mtds,"formMtds"=>$formMtds,  "offset"=>$offset, "limit"=>$limit, "sortInfo"=>$sortInfo);
            $return = $this->client->bmdAdvancedSearchFiles($params);
				return($return->searchResults->item);
        } catch (SoapFault $exception) {
            $methods = (get_object_vars($exception));
            $detail = null;
    ...
    function bmdAddSignature($cert, $signatureInput,$destFileId) {
        try {
            //właściwa struktura
            $params = array("cert"=>$cert,"signatureInput"=>$signatureInput ,"destFileId"=>$destFileId );
            $return = $this->client->bmdAddSignature($params);
...
//print_r($this->client);
	...
	function bmdCreateLink($cert, $mtds, $description, $linkName, $transactionId){

			try {
			$params = array("cert"=>$cert,"mtds"=>$mtds, "description"=>$description, "linkName"=>$linkName, "transactionId"=>$transactionId);
			$return = $this->client->bmdCreateLink($params);
			return($return->id);
			} catch (SoapFault $exception) {
				$methods = (get_object_vars($exception));
				$detail = null;
...
/*
 *Sekcja administrowania użytkownikami 
 * 
 * 
 */			
	function bmdRegisterNewUser($cert,$userAttributes){
			try {
			$params = array("cert"=>$cert,"userAttributes"=>$userAttributes);
			$return = $this->client->bmdRegisterNewUser($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdUpdateUserRights($cert,$userAttributes){
			try {				
			$params = array("cert"=>$cert,"userAttributes"=>$userAttributes);
			$return = $this->client->bmdUpdateUserRights($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdGetUsersList($cert){
			try {
			$params = array("cert"=>$cert);
			$return = $this->client->bmdGetUsersList($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdDeleteUser($cert,$certSerial,$certIssuer){
			try {
			$params = array("cert"=>$cert,"certSerial"=>$certSerial,"certIssuer"=>$certIssuer);
			$return = $this->client->bmdDeleteUser($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdDisableUser($cert,$certSerial,$certIssuer,$enable){
			try {
			$params = array("cert"=>$cert,"certSerial"=>$certSerial,"certIssuer"=>$certIssuer,"enable"=>$enable);
			$return = $this->client->bmdDisableUser($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	 ...
	 * bmdDeleteRole
	 */			
	function bmdGetRolesList($cert){
			try {
			$params = array("cert"=>$cert);
			$return = $this->client->bmdGetRolesList($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdRegisterNewRole($cert,$roleInfo){
			try {
			$params = array("cert"=>$cert,"roleInfo"=>$roleInfo);
			$return = $this->client->bmdRegisterNewRole($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdUpdateRole($cert,$roleInfo){
			try {
			$params = array("cert"=>$cert,"roleInfo"=>$roleInfo);
			$return = $this->client->bmdUpdateRole($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdDeleteRole($cert,$roleInfo){
			try {
			$params = array("cert"=>$cert,"roleInfo"=>$roleInfo);
			$return = $this->client->bmdDeleteRole($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	 ...
	 * bmdDeleteGroup
	 */
	function bmdGetGroupsList($cert){
			try {
			$params = array("cert"=>$cert);
			$return = $this->client->bmdGetGroupsList($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdRegisterNewGroup($cert,$groupInfo){
			try {
			$params = array("cert"=>$cert,"groupInfo"=>$groupInfo);
			$return = $this->client->bmdRegisterNewGroup($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdUpdateGroup($cert,$groupInfo){
			try {
			$params = array("cert"=>$cert,"groupInfo"=>$groupInfo);
			$return = $this->client->bmdUpdateGroup($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdDeleteGroup($cert,$groupInfo){
			try {
			$params = array("cert"=>$cert,"groupInfo"=>$groupInfo);
			$return = $this->client->bmdDeleteGroup($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
	...
	function bmdGetSecurityList($cert){
			try {
			$params = array("cert"=>$cert);
			$return = $this->client->bmdGetSecurityList($params);
			return($return->result);
			} catch (SoapFault $exception) {
			$methods = (get_object_vars($exception));
			$detail = null;
