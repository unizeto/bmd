...
draw_htmlhead('Wynik tworzenia katalogu w SOE SOAP WWW','');
...
$dirName=null;
$description=null;
if (!empty($_POST['dirname'])) {$dirName=$_POST['dirname'];}else{die('Nie podano nazwy katalogu');}
if (!empty($_POST['dirdesc'])) {$description=$_POST['dirdesc'];}else{die('Nie podano opisu');}
if (!empty($_POST['corespdir'])) {$mtds[] = array("mtdOid"=>"1.2.616.1.113527.4.3.2.5.7", "mtdValue"=>$_POST['corespdir'], "mtdStatement"=>0);}


$ret = $BMDSOAP->bmdCreateDirectory($cert, $mtds, $description, $dirName, $transID);
