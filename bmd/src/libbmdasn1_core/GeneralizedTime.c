...
static struct tm *localtime_r(const time_t *tloc, struct tm *result) {
	...
	int status;
	status=localtime_s(result,tloc);
	if( status != 0 )
		return NULL;
	return 0;
}
...
#else	/* HAVE_TM_GMTOFF */
	#ifdef BMD_ASN1_USE_TIMEZONE /*jesli koder GeneralizedTime ma uwzglêdniaæ przesuniêcie czasowe i modyfikowaæ datê*/
