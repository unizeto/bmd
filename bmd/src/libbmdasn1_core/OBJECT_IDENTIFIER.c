...
int OBJECT_IDENTIFIER_get_arcs(	OBJECT_IDENTIFIER_t *oid,
						void *arcs,
...
int i;
	...
	for(i = 0; i < oid->size; i++)
	{
		...
		if(num_arcs == 0)
		{
			...
			*(unsigned char *)((char *)arcs + ((*(char *)&LE)?0:(arc_type_size - 1))) = first_arc;
		...
		if(arcs < arcs_end)
		{
			if(OBJECT_IDENTIFIER_get_single_arc(&oid->buf[startn],i - startn + 1, add,arcs, arc_type_size))
			{
				return -1;
			}
			startn = i + 1;
