...
/* ------------------------------ */
 ...
 for(;;){
  ...
  longRet = soap_call_ns__awizoChangeAttachmentInfo(&soap2Eawizo,host,NULL, id, emailAttachment, result); 

  free0(host);

  if (longRet == SOAP_OK) {
	PRINT_INFO("Zmieniono informacje dotyczące załącznika o ID=%ld\n", id);
	return SOAP_OK;
  }
  else {
	if (iterator->prev){
