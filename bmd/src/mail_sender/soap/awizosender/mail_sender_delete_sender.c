...
PRINT_INFO("AWIZOSOAPSERVERINF Delete Awizo Sender.\n" );

if (_GLOBAL_shptr->awizoAction != queueNothing) {
	PRINT_INFO("AWIZOSOAPSERVERINF Can't delete awizo sender, server already in use!\n" );
	return soap_receiver_fault(soap, "Can't delete awizo sender" , "Awizo serwer already in use");
}

if (id <= 0) {
	PRINT_ERROR("Niepoprawny identyfikator nadawcy. Error = %d\n", BMD_ERR_OP_FAILED);
	return soap_receiver_fault(soap, "Illegal value of sender id", "sender id value <= 0");
}



getTime(&stringTime);
asprintf( &stringSQL, "UPDATE mailing_message_senders SET deleted = '%s' WHERE id = %ld", stringTime, id);
