...
int main(int argc, char *argv[])
{
	...
	long result = 0;
	long i = 0;
	struct ns__awizoConfig* config = NULL;
	...
	config = (struct ns__awizoConfig*)calloc(1,sizeof(struct ns__awizoConfig));

	//config->smtpAddr = "";
	//config->smtpPort = "25";
	//config->smtpUser = "tklimek";
	config->sendingtime = "60";
	...
	status = soap_call_ns__changeConfiguration(&soap, host, NULL, config, &result);
