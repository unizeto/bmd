...
/* ****************************************** */
 ...
 /* *********************************** */
   ...
   if (mode == modeSMS || mode == modeSMSEMAIL) {
	saveRaportToDb(hMessage,_GLOBAL_awizoConfig.dbase_handler, *MsgID, atol(templateId), 1, "Prepared successfully", modeSMS);
 ...
 /* ********************************** */
	...
	if (longRet != BMD_OK) {
		...
		sigemptyset(&_GLOBAL_mask_set);
 		sigprocmask(SIG_SETMASK, &_GLOBAL_mask_set, NULL);
		saveRaportToDb(hMessage,_GLOBAL_awizoConfig.dbase_handler, *MsgID, atol(templateId), longRet, "eAwizo prepared failed", modeEMAIL);
		xmlFreeDoc(hMessage);
		free0(templateId);
	...
	if (_GLOBAL_awizoConfig.awizoTable == NULL) {
		...
		sigemptyset(&_GLOBAL_mask_set);
 		sigprocmask(SIG_SETMASK, &_GLOBAL_mask_set, NULL);
		saveRaportToDb(hMessage,_GLOBAL_awizoConfig.dbase_handler, *MsgID, 1, BMD_ERR_MEMORY, "eAwizo table memory error", modeEMAIL);
		xmlFreeDoc(hMessage);
		free0(templateId);
		sem_post(&(_GLOBAL_shptr->clientMutex));
