...
int ns__awizoDeleteTemplate(struct soap* soap, long id, long* result)
{
...
/* ------------------------------------- */
 ...
 PRINT_INFO("AWIZOSOAPSERVERINF Delete Awizo Template.\n" );

 if (_GLOBAL_shptr->awizoAction != queueNothing) {
	PRINT_INFO("AWIZOSOAPSERVERINF Can't delete template, server already in use!\n" );
	return soap_receiver_fault(soap, "Can't delete template" , "Awizo serwer already in use");
 }

 if (id <= 0) {
	PRINT_ERROR("Niepoprawny identyfikator szablonu. Error = %d\n", BMD_ERR_OP_FAILED);
	return soap_receiver_fault(soap, "Illegal template id value", "template id value <= 0");
 }

 getTime(&stringTime);
 asprintf( &stringSQL, "UPDATE mailing_email_template SET deleted = '%s' WHERE id = %ld", id);
