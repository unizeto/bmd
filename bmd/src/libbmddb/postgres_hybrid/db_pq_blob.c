/* komentarz:
BLOB_IN_BYTEA_HYBRID_BYTEA_OFF jest niezdefiniowan¹ flag¹, 
która wstawiona w miejsce flagi BLOB_IN_BYTEA wy³¹cza odczytywanie z BYTEA (na rzecz odczytywania z pg_large_objects).
brak jej definicji jest wiêc zabiegiem celowym
*/
...
#define SBT(hDB,db_conn) if( db_conn->transaction_present != 1 ) \
						 { \
							...
							if( s != BMD_OK ) \
							{ \
								PRINT_ERROR("Cannoc start transaction for BLOB operation\n"); \
...
#else
	/* pobierz sekwencjê numeru obiektu binarnego lobjId (blob_number)*/
...
long postgres_bmd_db_import_blob(void *hDB,char *blob_number,GenBuf_t **dataGenBuf)
{
...
#ifndef BLOB_IN_BYTEA_HYBRID_BYTEA_OFF
	long lobj_fd		= 0;
	long nbytes			= 0;
	long lobj_size		= 0;
#else
	...
	int err				= 0;
	Oid in_oid[]		={0};			/* 0 oznacza, ¿e PSQL powinien sam rozpoznaæ OIDy*/ 
	...
	params_length[0] = (int)strnlen(blob_number,21);		// jesli stosujemy partycjonowanie tabeli tblob - tu zawsze powinno byæ 19 znaków (zakres BIGINT postgresa w zapisie dziesi¹tkowym)

	/* pobierz BLOB z tabeli tblob (z partycji wybranej przez funkcjê sk³adow¹ sel_tblob */
	pq_db_res = PQexecParams(db_conn->pg_conn,   
	...
	{ 
		...
		if (0 == PQgetisnull(pq_db_res,0,0))	//w wierszu s¹ niepuste dane binarne - coœ siê odczyta³o
		{
...
#ifdef BLOB_IN_BYTEA_HYBRID_BYTEA_OFF
...
#else
	//TODO w 2.3 dopisaæ asercje (which_piece != LOB_FIRST_PIECE && raster_id == *blob_fd)
	//TODO w 2.3 dopisaæ asercje osiagniecia last_chunk i proby pobrania LOB_NEST_PIECE
