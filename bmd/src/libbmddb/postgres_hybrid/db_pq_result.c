							 ...
							 EnumDbFetchMode_t fetch_mode,long *successfully_fetched)
{
	...
	if( PQgetisnull(db_res->pg_res, which_row_next, which_column) == 1 ) // NULL
	{
		*value = NULL;
	}
	else // nie NULL
	{
		*value=strdup(PQgetvalue(db_res->pg_res, which_row_next, which_column));
	}
						   ...
						   EnumDbFetchMode_t fetch_mode,long *successfully_fetched)
{
	...
	*rowstruct = (db_row_strings_t *) malloc ( sizeof(db_row_strings_t) );
	...
	{
		if(PQgetisnull(db_res->pg_res, which_row_next, i) == 1) // NULL
		{
			(*rowstruct)->colvals[i] = NULL;
		}
		else // nie NULL
		{
			(*rowstruct)->colvals[i] = strdup(PQgetvalue(db_res->pg_res, which_row_next, i));
		}
	}
